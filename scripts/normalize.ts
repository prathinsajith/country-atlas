...
import type { Country, GeoData, Currency, Flag, Timezone, Postal, Formats, Domains, ISO } from '../src/types';
...
interface RawCountry {
    name: { common: string; official: string; native: Record<string, { common: string; official: string }> };
    cca2: string;
    cca3: string;
    ccn3: string;
...
async function normalize() {
    ...
    const normalizedCountries: Country[] = await Promise.all(rawData.map(async (raw: any) => {
        // Mapping logic
        const iso: ISO = {
            alpha2: raw.cca2,
            alpha3: raw.cca3,
        ...
        try {
            if (iso.alpha2) {
                const svgPath = path.join(__dirname, '../data/flags/optimized', `${iso.alpha2.toLowerCase()}.svg`);
    ...
    const minify = (obj: any): any => {
        if (obj === null || obj === undefined || (typeof obj === 'string' && obj === '')) return undefined;
    ...
    await fs.writeFile(path.join(NORMALIZED_DIR, 'countries.json'), JSON.stringify(minify(normalizedCountries), null, 2)); // Keeping formatted for readability in dev, but dist will be minified. 
